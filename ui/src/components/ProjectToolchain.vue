...
async function decide(id: string, approve: boolean) {
  busy.value = true;
  try {
    await api(`/project-toolchain/approvals/${id}`, "POST", {approve, allow_additional_packages: additional.value},
...
<template>
  <section v-if="project" class="panel">
    ...
    <section v-for="request in requests" :key="request.request_id" class="notice">
      ...
      <template v-if="request.status === 'awaiting_approval'">
      ...
      <button :disabled="!editable || busy || !operator || !secret || request.installing" @click="decide(request.request_id, true)">Approve exact manifest and install</button>
      <button :disabled="!editable || busy || !operator || !secret || request.installing" @click="decide(request.request_id, false)">Deny</button>
      </template>
    </section>
  </section>
</template>
